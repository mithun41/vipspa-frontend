...
const ManageBlogs = () => {
  ...
  const handleSectionUpdate = async (e) => {
    ...
    try {
      const res = await fetch("http://127.0.0.1:8000/api/vipspa/blog-sections/1/", { // ধরে নিচ্ছি ID ১
        method: "PATCH",
        headers: { 
    ...
    } catch (err) { alert("Failed to update section info"); }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    } catch (err) { alert("Post action failed!"); }
  };
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("adminToken");
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/vipspa/blog-posts/${id}/`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` },
      });
    ...
    <AdminLayout>
      <div className="container-fluid py-4">
        ...
        <div className="card shadow-sm border-0 p-4 mb-4">
          <h6 className="fw-bold text-muted text-uppercase mb-3">Blog Section Header</h6>
        ...
        <div className="row">
          ...
          <div className="col-md-4">
            <div className="card shadow-sm border-0 p-4 sticky-top" style={{ top: "20px" }}>
              <h5 className="fw-bold mb-4 text-primary">{isEditing ? "Edit Blog Post" : "Add New Post"}</h5>
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                  ...
                  {preview && <img src={preview} className="w-100 rounded border" style={{height: "120px", objectFit: "cover"}} alt="preview" />}
                </div>
                    ...
                    <tr key={post.id}>
                      <td className="px-4">
                        <div className="d-flex align-items-center gap-3">
                          <img src={post.image} className="rounded" style={{ width: "60px", height: "40px", objectFit: "cover" }} alt="" />
                          <div className="fw-bold text-truncate" style={{maxWidth: "250px"}}>{post.title}</div>
                        </div>
                      </td>
                      <td><span className="badge bg-info-subtle text-info border border-info-subtle">{post.category}</span></td>
                      <td className="text-end px-4">
                        <div className="btn-group">
                          <button onClick={() => handleEditClick(post)} className="btn btn-sm btn-outline-primary">Edit</button>
