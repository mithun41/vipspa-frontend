...
function ServiceAdmin() {
    const [services, setServices] = useState({ general_services: [], top_services: [] });
   ...
   const fetchServices = async () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                alert(isEditing ? "Update successfully!" : "New service created successfully!");
                resetForm();
                fetchServices();
            } else {
    ...
    const resetForm = () => {
        setFormData({ id: null, title: '', description: '', service_type: 'general', is_active: true, icon_image: null, duration: '', category: 1 });
        setPreview(null);
        setIsEditing(false);
    };
    ...
    return (
        <AdminLayout>
            <div className="container-fluid py-4">
                <h3 className="mb-4 fw-bold">{isEditing ? "📝 Edit Service" : "✨ Add New Service"}</h3>
                ...
                <div className="row">
                        ...
                        <div className="card shadow-sm border-0 p-4 sticky-top" style={{ top: "20px" }}>
                            <form onSubmit={handleSubmit}>
                                ...
                                <div className="mb-3">
                                    ...
                                    <input className="form-control" value={formData.duration} onChange={e => setFormData({...formData, duration: e.target.value})} placeholder="e.g. 1 hr" />
                                </div>
                                <div className="mb-3">
                                ...
                                <div className="mb-3">
                                    ...
                                    <input type="file" className="form-control mb-2" onChange={e => {
                                        ...
                                        if(file) { setFormData({...formData, icon_image: file}); setPreview(URL.createObjectURL(file)); }
                                    }} />
                                    {preview && <img src={preview} className="rounded border mt-2" style={{width: "60px", height: "60px", objectFit: "cover"}} alt="preview" />}
                                </div>
                                <div className="form-check form-switch mb-4">
                                    <input type="checkbox" className="form-check-input" checked={formData.is_active} onChange={e => setFormData({...formData, is_active: e.target.checked})} id="activeCheck" />
                                    <label className="form-check-label small fw-bold" htmlFor="activeCheck">Show on Website</label>
                                </div>
                                <button type="submit" className={`btn w-100 fw-bold ${isEditing ? "btn-primary" : "btn-warning"}`}>
                                    {isEditing ? "UPDATE SERVICE" : "PUBLISH SERVICE"}
                                </button>
                                {isEditing && <button type="button" onClick={resetForm} className="btn btn-link w-100 text-muted mt-2">Cancel</button>}
                            </form>
                        </div>
                    </div>
                    ...
                    <div className="col-md-8">
                        <div className="card shadow-sm border-0 mb-4">
                            <div className="card-header bg-white py-3"><h6 className="mb-0 fw-bold text-primary">⭐ TOP SERVICES</h6></div>
                            <ServiceListTable list={services.top_services} onEdit={handleEditClick} onDelete={handleDelete} loading={loading} />
                        </div>
                        <div className="card shadow-sm border-0">
                            <div className="card-header bg-white py-3"><h6 className="mb-0 fw-bold text-info">📂 GENERAL SERVICES</h6></div>
                            <ServiceListTable list={services.general_services} onEdit={handleEditClick} onDelete={handleDelete} loading={loading} />
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <tr key={s.id}>
                        <td className="ps-4">
                            <div className="d-flex align-items-center gap-3">
                                <img src={s.icon_image} className="rounded border bg-light" style={{ width: "45px", height: "45px", objectFit: "contain" }} alt="" />
                                <div>
                        ...
                        <td className="text-end pe-4">
                            <div className="btn-group shadow-sm">
                                <button onClick={() => onEdit(s)} className="btn btn-sm btn-outline-primary">Edit</button>
                                <button onClick={() => onDelete(s.id)} className="btn btn-sm btn-outline-danger">Delete</button>
                            </div>
                        </td>
                    </tr>
                ))}
