...
const ManageTestimonials = () => {
 ...
 const handleSubmit = async (e) => {
  ...
  const handleDelete = async (id) => {
    ...
    } catch (err) { alert("Delete failed error!"); }
  };
            ...
            <div className="card shadow-sm border-0 p-4 sticky-top" style={{ top: "20px" }}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="mb-3">
                  ...
                  <textarea rows="4" className="form-control" value={formData.comment} onChange={e => setFormData({...formData, comment: e.target.value})} required />
                </div>
              ...
              <table className="table table-hover align-middle mb-0">
                ...
                <tbody>
                  {loading ? (
                      ...
                      <tr key={item.id}>
                        <td className="px-4">
                          <div className="d-flex align-items-center gap-3">
                            <img src={item.photo} className="rounded-circle" style={{ width: "40px", height: "40px", objectFit: "cover" }} alt="" />
                            <div className="fw-bold">{item.name}</div>
                          </div>
                        </td>
                        <td><div className="text-warning">{"★".repeat(item.rating)}</div></td>
                    ...
                    <tr><td colSpan="3" className="text-center py-5">No items found.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
