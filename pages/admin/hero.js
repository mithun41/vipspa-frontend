...
const ManageHero = () => {
  ...
  const handleDelete = async (id) => {
    ...
    const token = localStorage.getItem("adminToken");
    try {
      const res = await fetch(`http://127.0.0.1:8000/api/vipspa/hero-slides/${id}/`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` },
      });
      if (res.ok) {
        ...
        <div className="row">
          ...
          <div className="col-md-4">
            <div className="card shadow-sm border-0 p-4 sticky-top" style={{ top: "20px" }}>
              <h5 className="fw-bold mb-3">{isEditing ? "📝 Edit Slide" : "➕ Add New Slide"}</h5>
              <form onSubmit={handleSubmit}>
                <div className="mb-2">
                  <label className="small fw-bold">Stroke Text</label>
                  <input type="text" className="form-control form-control-sm" value={formData.stroke_text} onChange={e => setFormData({...formData, stroke_text: e.target.value})} />
                </div>
                <div className="mb-2">
                ...
                <div className="mb-2">
                  ...
                  <textarea rows="2" className="form-control form-control-sm" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} />
                </div>
                <div className="row">
                  <div className="col-6 mb-2">
                    <label className="small fw-bold">Button Text</label>
                    <input type="text" className="form-control form-control-sm" value={formData.button_text} onChange={e => setFormData({...formData, button_text: e.target.value})} />
                  </div>
                  <div className="col-6 mb-2">
                    <label className="small fw-bold">Order</label>
                    <input type="number" className="form-control form-control-sm" value={formData.order} onChange={e => setFormData({...formData, order: e.target.value})} />
                  </div>
                </div>
                ...
                <div className="mb-2">
                  ...
                  <input type="file" className="form-control form-control-sm" onChange={e => {
                    ...
                    if(file) { setFormData({...formData, background_image: file}); setBgPreview(URL.createObjectURL(file)); }
                  }} />
                  {bgPreview && <img src={bgPreview} className="mt-1 rounded border" style={{ height: "40px", width: "100%", objectFit: "cover" }} />}
                </div>
                ...
                <div className="mb-3">
                  ...
                  <input type="file" className="form-control form-control-sm" onChange={e => {
                    const file = e.target.files[0];
                    if(file) { setFormData({...formData, main_image: file}); setMainPreview(URL.createObjectURL(file)); }
                  }} />
                  {mainPreview && <img src={mainPreview} className="mt-1 rounded border" style={{ height: "40px", objectFit: "contain" }} />}
                </div>

                <button type="submit" className={`btn w-100 fw-bold ${isEditing ? "btn-primary" : "btn-success"}`}>
                   {isEditing ? "Update Slide" : "Save Slide"}
                </button>
                {isEditing && <button type="button" className="btn btn-link w-100 text-danger mt-1 text-decoration-none" onClick={resetForm}>Cancel</button>}
              </form>
            </div>
          </div>
                      ...
                      <tr key={s.id}>
                        <td>
                          <div className="d-flex gap-1">
                             <img src={s.background_image} title="Background" style={{ width: "50px", height: "30px", objectFit: "cover", borderRadius: "4px" }} />
                             <img src={s.main_image} title="Main Image" style={{ width: "30px", height: "30px", objectFit: "contain", background: "#eee", borderRadius: "4px" }} />
                          </div>
                        </td>
                        <td>
                          <div className="small fw-bold">{s.title}</div>
                          <div className="text-muted" style={{ fontSize: "11px" }}>{s.stroke_text}</div>
                        </td>
                        <td>{s.order}</td>
